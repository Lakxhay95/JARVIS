...
import random # Import random for importing random choices.
import asyncio # Import asyncio for asynchronous operations.
import edge_tts # Import edge_tts for text-to-specch functionality.
...
async def TextToAudioFile(text) -> None:
    file_path = r"Data\speech.mp3" # Define the path where speech file will be saved.

    if os.path.exists(file_path): # Check if the file already exists.
        os.remove(file_path) # If it exists, remove it to avoid overwriting errors.
    
    # Create the communicate objectto generate speech.
...
def TTS(Text, func=lambda r=None: True):
    while True:
        try:
            ...
            # Load the generated speech fileinto pygame mixer.
            ...
            while pygame.mixer.music.get_busy():
                if func() == False: # Check if the external function returns false.
                    break
                pygame.time.Clock().tick(10) # Limit the loop to 10 clicks per second.
...
# Female voice(INDIAN) = en-IN-NeerjaNeural
# Male voice(INDIAN) = en-IN-PrabhatNeural
