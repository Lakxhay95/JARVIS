# OpenApp folder me mistake h
# Import required libraries.
from AppOpener import close, open as appopen # Import functions to open and close apps.
from webbrowser import open as webopen # Import web borwser functionality.
...
GroqAPIKey = env_vars.get("GroqAPIKey") # Retieve the Groq API key.
...
# Predefined professionals responses for user interactions.
professional_response = [
    "Your satissfaction is my top priority; feel free to reach out if there's anyhting else I can help you with.",
...
SystemChatBot = [{"role": "system", "content": f"Hello, I am {os.environ['Username']}, You're a content writer. You can write content like letters, codes, applications, essays, notes, songs, poems etc."}]

# Function to perform google search.
def GoogleSearch(Topic):
    search(Topic) # Use pywhatki's search function to perform a Google search.
    return True # Indicate success.

# Functio ot generate content using AI and save it to a file.
def Content(Topic):

    # Nested function to open.
    ...
    def ContentWriterAI(prompt):
        ...
        completion = client.chat.completions.create(
            ...
            messages=SystemChatBot + messages, # Include the system instructions and chat history.
            max_tokens=2048, # Limit the maximum tokens in the response.
            temperature=0.7, # Adjust response randomness.
            top_p=1, # Use nucleus sampling for response diversity.
            stream=True, # Enabling stream response.
        ...
        messages.append({"role": "assistant", "content": Answer}) # Add AI's response to messages.
        return Answer
...
def OpenApp(app, sess=requests.session()):
    ...
    try:
        ...
        def extract_links(html):
            ...
            return [link.get('href') for link in links] # Return the GET request.
        
        # Nested function to perform a Google search and retireve HTML.
...
def System(command):
    ...
    def volume_down():
        keyboard.press_and_release("volume down") # Simulate the volume downkey press.
...
#     asyncio.run(Automation(["open netflix", "open instagram", "open settings", "play darkside", "content song for me"]))